...
var db = new Datastore({ filename: './bundles/eol-wordfilter/lib/wordfilter.db', autoload: true });
var Q = require('q');
...
Wordfilter.prototype.blacklisted = function (string) {
    this.getList()
        .then(function (blacklist) {
            ...
            console.log('[eol-wordfilter] Failed to load list of banned phrases: ' + reason);
        });
    return false;
};
...
Wordfilter.prototype.getList = function() {
    ...
    db.findOne({ _id: 'blacklist' }, function (err, blacklist) {
        if (err) {
            ...
            deferred.resolve(blacklist.words.sort());
        }
    });
    return deferred.promise;
};
